...
pub fn Toast(cx: Scope) -> Element {
    ...
    cx.render(rsx! {
        div {
            class: "@apply toast-message fixed z10 flex {toast} w-full items-center justify-center ease-in-out",
            div {
                class: "bg-green bg-opacity-50 filter backdrop-blur text-white rounded p4",
                "✅ {message}"
            }
        }
    })
